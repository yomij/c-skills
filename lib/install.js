...
function resolveInstallHost(host) {
  const resolved = host || DEFAULT_HOST;

  if (resolved !== 'codex' && resolved !== 'claude') {
    throw new Error(`Invalid install host: ${host}. Use "codex" or "claude".`);
  }

  return resolved;
}

function getGlobalSkillsDir(host = DEFAULT_HOST) {
  if (resolveInstallHost(host) === 'claude') {
...
function getProjectSkillsDir(host = DEFAULT_HOST, cwd = process.cwd()) {
  const dirName = resolveInstallHost(host) === 'claude' ? '.claude' : '.codex';
  return path.join(cwd, dirName, 'skills');
}
...
function resolveInstallScope(scope) {
  const resolved = scope || DEFAULT_SCOPE;

  if (resolved !== 'global' && resolved !== 'project') {
...
function resolveTargetRoot(options = {}, cwd = process.cwd()) {
  if (options.dest) {
    return path.resolve(expandHome(options.dest));
  }

  const scope = resolveInstallScope(options.scope);
  const host = resolveInstallHost(options.host);

  if (scope === 'project') {
    return path.resolve(getProjectSkillsDir(host, cwd));
  }

  return path.resolve(getGlobalSkillsDir(host));
}

function describeHost(options = {}) {
  if (options.dest) {
    return 'selected host';
  }

  return resolveInstallHost(options.host) === 'claude' ? 'Claude Code' : 'Codex';
}

async function listBundledSkills() {
  const entries = await fs.readdir(BUNDLED_SKILLS_DIR, { withFileTypes: true }).catch(() => []);
  const skillNames = [];
  ...
  for (const entry of entries) {
    ...
    const skillDir = path.join(BUNDLED_SKILLS_DIR, entry.name);
    const skillFile = await fs.stat(path.join(skillDir, 'SKILL.md')).catch(() => null);

    if (skillFile && skillFile.isFile()) {
...
async function installBundledSkill(options) {
  const targetRoot = resolveTargetRoot(options);
  const targetName = options.name || options.skill;
  const sourceDir = path.join(BUNDLED_SKILLS_DIR, options.skill);

  validateSkillName(options.skill);
  validateSkillName(targetName);

  await ensureSkillDirectory(sourceDir).catch(async () => {
    const bundledSkills = await listBundledSkills();
    const availableList = bundledSkills.length > 0 ? bundledSkills.join(', ') : '(none bundled)';
    throw new Error(`Bundled skill not found: ${options.skill}. Available bundled skills: ${availableList}`);
  });

  if (options.dryRun) {
    console.log(`Bundled skill: ${options.skill}`);
    console.log(`Source: ${sourceDir}`);
    console.log(`Host: ${resolveInstallHost(options.host)}`);
    console.log(`Scope: ${options.dest ? 'custom' : resolveInstallScope(options.scope)}`);
    console.log(`Destination: ${path.join(targetRoot, targetName)}`);
    return;
  }

  console.log(`Installing bundled skill ${options.skill}...`);
  await copySkill(sourceDir, path.join(targetRoot, targetName));
  console.log(`Installed skill to ${path.join(targetRoot, targetName)}`);
  console.log(`Restart ${describeHost(options)} to pick up new skills.`);
}

async function installGitHubSkill(options) {
...
async function installSkill(options) {
  ...
  await installGitHubSkill(options);
}
